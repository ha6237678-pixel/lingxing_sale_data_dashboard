...
export async function getProductLineSettlementRows(
  ...
  const selectSql = `select
      ...
      coalesce(abs(sum(total_sales_refunds)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as refunds_rate,
      coalesce(abs(sum(total_ads_cost)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as ads_cost_rate,
      coalesce(abs(sum(promotional_rebates)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as discount_rate,
      coalesce(abs(sum(total_ads_cost)) / nullif(sum(total_ads_sales), 0), 0) as acos,
      coalesce(abs(sum(total_ads_cost)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as tacos,
      coalesce(abs(sum(cg_transport_costs)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as first_trip_rate,
      coalesce(abs(sum(cg_price)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as purchase_rate,
      coalesce(abs(sum(fba_delivery_fee)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as fba_rate,
      coalesce(abs(sum(total_storage_fee)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as storage_rate,
      coalesce(abs(sum(platform_fee)) / nullif(sum(total_sales_amount_with_tax), 0), 0) as platform_rate,
