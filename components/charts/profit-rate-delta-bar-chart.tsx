...
export function ProfitRateDeltaBarChart({
  ...
  const chartData = profitRateSeries.map((item) => ({
    name: item.name,
    value: current[item.key] - previous[item.key],
    color: item.color,
  }));
    ...
    <section className="mt-5 border border-line bg-white p-4 shadow-panel">
      ...
      <div className="h-96">
        <ResponsiveContainer height="100%" width="100%">
          <BarChart data={chartData} layout="vertical" margin={{ left: 16, right: 24, top: 10, bottom: 0 }}>
            ...
            <ReferenceLine stroke="#94a3b8" type="number" x={0} />
